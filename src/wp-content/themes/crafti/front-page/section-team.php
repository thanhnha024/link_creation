<div class="front_page_section front_page_section_team<?php
	$crafti_scheme = crafti_get_theme_option( 'front_page_team_scheme' );
	if ( ! empty( $crafti_scheme ) && ! crafti_is_inherit( $crafti_scheme ) ) {
		echo ' scheme_' . esc_attr( $crafti_scheme );
	}
	echo ' front_page_section_paddings_' . esc_attr( crafti_get_theme_option( 'front_page_team_paddings' ) );
	if ( crafti_get_theme_option( 'front_page_team_stack' ) ) {
		echo ' sc_stack_section_on';
	}
?>"
		<?php
		$crafti_css      = '';
		$crafti_bg_image = crafti_get_theme_option( 'front_page_team_bg_image' );
...
<?php
	// Add anchor
	$crafti_anchor_icon = crafti_get_theme_option( 'front_page_team_anchor_icon' );
	$crafti_anchor_text = crafti_get_theme_option( 'front_page_team_anchor_text' );
...
?>
	...
	<?php
	if ( crafti_get_theme_option( 'front_page_team_fullheight' ) ) {
	...
	?>
			...
			<?php
			$crafti_css      = '';
			$crafti_bg_mask  = crafti_get_theme_option( 'front_page_team_bg_mask' );
			$crafti_bg_color_type = crafti_get_theme_option( 'front_page_team_bg_color_type' );
			if ( 'custom' == $crafti_bg_color_type ) {
				$crafti_bg_color = crafti_get_theme_option( 'front_page_team_bg_color' );
	...
	>
		<div class="front_page_section_content_wrap front_page_section_team_content_wrap content_wrap">
			<?php
			// Caption
			$crafti_caption = crafti_get_theme_option( 'front_page_team_caption' );
			if ( ! empty( $crafti_caption ) || ( current_user_can( 'edit_theme_options' ) && is_customize_preview() ) ) {
				?>
				<h2 class="front_page_section_caption front_page_section_team_caption front_page_block_<?php echo ! empty( $crafti_caption ) ? 'filled' : 'empty'; ?>"><?php echo wp_kses( $crafti_caption, 'crafti_kses_content' ); ?></h2>
				<?php
			}

			// Description (text)
			$crafti_description = crafti_get_theme_option( 'front_page_team_description' );
			if ( ! empty( $crafti_description ) || ( current_user_can( 'edit_theme_options' ) && is_customize_preview() ) ) {
				?>
				<div class="front_page_section_description front_page_section_team_description front_page_block_<?php echo ! empty( $crafti_description ) ? 'filled' : 'empty'; ?>"><?php echo wp_kses( wpautop( $crafti_description ), 'crafti_kses_content' ); ?></div>
				<?php
			}
			...
			<div class="front_page_section_output front_page_section_team_output">
				...
				if ( is_active_sidebar( 'front_page_team_widgets' ) ) {
					...
					if ( ! crafti_exists_trx_addons() ) {
						...
						crafti_customizer_need_widgets_message( 'front_page_team_caption', 'ThemeREX Addons - Team' );
					}
				}
				?>
			</div>
