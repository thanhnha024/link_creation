<?php
/**
 * Widget: Properties Sort (Shortcodes)
 *
		...
		if ( is_object( $wp_widget_factory ) && isset( $wp_widget_factory->widgets, $wp_widget_factory->widgets[ $type ] ) ) {
			$output = '<div' . ($id ? ' id="'.esc_attr($id).'"' : '')
							. ' class="widget_area sc_widget_properties_sort' 
								. (trx_addons_exists_vc() ? ' vc_widget_properties_sort wpb_content_element' : '') 
...
// Add shortcode [trx_widget_properties_sort]
if (!function_exists('trx_addons_sc_widget_properties_sort_add_shortcode')) {
	function trx_addons_sc_widget_properties_sort_add_shortcode() {
		add_shortcode("trx_widget_properties_sort", "trx_addons_sc_widget_properties_sort");
	}
	add_action('init', 'trx_addons_sc_widget_properties_sort_add_shortcode', 20);
}
