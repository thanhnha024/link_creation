<?php
/**
 * Widget: Properties Search (Advanced search form) (Shortcodes)
 *
		...
		if ( is_object( $wp_widget_factory ) && isset( $wp_widget_factory->widgets, $wp_widget_factory->widgets[ $wtype ] ) ) {
			$output = '<div' . ($id ? ' id="'.esc_attr($id).'"' : '')
							. ' class="widget_area sc_widget_properties_search' 
								. (trx_addons_exists_vc() ? ' vc_widget_properties_search wpb_content_element' : '') 
...
// Add shortcode [trx_widget_properties_search]
if (!function_exists('trx_addons_sc_widget_properties_search_add_shortcode')) {
	function trx_addons_sc_widget_properties_search_add_shortcode() {
		add_shortcode("trx_widget_properties_search", "trx_addons_sc_widget_properties_search");
	}
	add_action('init', 'trx_addons_sc_widget_properties_search_add_shortcode', 20);
}
