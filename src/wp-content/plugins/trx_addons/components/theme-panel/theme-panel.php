			...
			if ( !empty( $nonce ) ) {
				...
				// Check code
					...
					if ( $result['data'] === 1 ) {
						...
						trx_addons_set_admin_message( sprintf( __("Sorry, the theme is not activated! Reason: %s", 'trx_addons'),
		...
		<div id="trx_addons_theme_panel" class="trx_addons_theme_panel">
			...
			<div class="trx_addons_result">
				...
				if ( ! empty( $msg ) ) {
					...
					?><div id="trx_addons_theme_panel_message" class="<?php echo esc_attr( $type ); ?>"><p><?php echo wp_kses_data( $msg ); ?></p></div><?php
					// Popup message box
					?>
					<script>
				...
				if ( $theme_status == 'active' && trx_addons_is_theme_activated(true) ) {
					...
					?><div class="trx_addons_theme_panel_section_license">
						...
						<div class="trx_addons_theme_panel_section_license_description">
							...
							<p class="trx_addons_theme_panel_section_license_note"><?php echo wp_kses_data( __( '<b>Please Note! Reinstallation on the same domain still requires disconnection of the purchase key.</b> Before taking any action that require another theme activation (WordPress re-installation, database cleaning, etc), you need to deactivate the theme by clicking the "Disconnect domain" button.', 'trx_addons' ) ); ?></p>
