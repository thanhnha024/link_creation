<?php
/**
 * Widget: Posts by Rating (Shortcodes)
 *
...
if ( !function_exists( 'trx_addons_sc_widget_rating_posts' ) ) {
	function trx_addons_sc_widget_rating_posts($atts, $content = ''){	
		$atts = trx_addons_sc_prepare_atts('trx_widget_rating_posts', $atts, trx_addons_sc_common_atts('id', array(
			// Individual params
			"title" => "",
			"number" => 4,
			"post_type" => '',
...
// Add shortcode [trx_widget_rating_posts]
if (!function_exists('trx_addons_sc_widget_rating_posts_add_shortcode')) {
	function trx_addons_sc_widget_rating_posts_add_shortcode() {
		add_shortcode("trx_widget_rating_posts", "trx_addons_sc_widget_rating_posts");
	}
	add_action('init', 'trx_addons_sc_widget_rating_posts_add_shortcode', 20);
}
