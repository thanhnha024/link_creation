...
if ( ! function_exists('trx_addons_elm_add_parallax_params_to_widgets' ) ) {
	...
	function trx_addons_elm_add_parallax_params_to_widgets($element, $section_id, $args) {
		...
		if ( in_array( $element->get_name(), array( 'section', 'column', 'container', 'common' ) ) && $section_id == '_section_responsive' ) {
			...
			foreach ( $bp as $resp => $max ) {
				...
				foreach ( $params as $param => $default ) {
					...
					$data[ $param_name ] = isset( $settings["parallax_{$param_name}"] ) && $settings["parallax_{$param_name}"]['size'] !== ''
											...
											: ( empty( $suffix ) ? $default['default'] : $data[ $param_prev ] );
					if ( isset( $default['unit'] ) ) {
						$data[ "{$param_name}_unit" ] = isset( $settings["parallax_{$param_name}"]['unit'] ) && $settings["parallax_{$param_name}"]['size'] !== ''
											...
											: ( empty( $suffix ) ? $default['unit'] : $data[ "{$param_prev}_unit" ] );
					}
				}
