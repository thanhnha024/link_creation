...
function trx_addons_is_wp_editor_inited() {

	"use strict";

	return trx_addons_get_wp_editor().trx_addons_inited === true;
}
...
// Add params to the new section
//-------------------------------------------
function trx_addons_gutenberg_add_param_section(props, slug, title, params) {
	...
	return el( wp.element.Fragment, { key: props.name + '-' + slug + '-params' },
				el( trx_addons_get_wp_editor().InspectorControls, {},
					el( wp.components.PanelBody, { title: title },
						el( 'div', { className: 'components-panel__body-fieldset' },
							trx_addons_gutenberg_add_params( params, props )
						)
					)
...
function trx_addons_gutenberg_get_child_attr(props, return_array) {
	...
	var items = [];
	if ( props.innerBlocks && props.innerBlocks.length ) {
...
jQuery( window ).on( 'load', function() {
	...
	function trx_addons_gutenberg_editor_init() {
		...
		// Mark WordPress object as inited
		trx_addons_get_wp_editor().trx_addons_inited = true;
	}
} );
