...
if ( ! function_exists( 'trx_addons_enqueue_slider' ) ) {
	...
	function trx_addons_enqueue_slider( $engine = 'swiper', $force = false ) {
		...
		if ( ! $force && ! empty( $loaded[ $engine ] ) ) {
			return;
		}

		// Load once and only in the frontend
		if ( apply_filters( 'trx_addons_filter_prevent_load_frontend_scripts_in_editor', true, 'slider', $engine ) && strpos( trx_addons_get_current_url(), '/wp-admin/post.php' ) !== false ) {
			return;
		}
