...
if ( ! function_exists( 'trx_addons_image_effects_remove_lazy_load_from_page_content' ) ) {
	...
	function trx_addons_image_effects_remove_lazy_load_from_page_content() {
		ob_start( function( $html ) {
			$html = preg_replace( '/(<div[^>]*data-image-effect[^>]*>[\s]*(<div[^>]*>[\s]*<div[^>]*>[\s]*)?<img[^>]*)(loading="lazy")/i', '${1}loading="eager"', $html );
			$html = preg_replace( '/(<div[^>]*data-image-effect[^>]*>[\s]*(<div[^>]*>[\s]*<div[^>]*>[\s]*)?<img[^>]*)(decoding="async")/i', '${1}decoding="sync"', $html );
			$html = preg_replace_callback( '/<img[^>]*trx_addons_image_effect_original_image[^>]*>/i', function( $matches ) {
				if ( ! empty( $matches[0] ) ) {
					$matches[0] = str_replace( 'loading="lazy"', 'loading="eager"', $matches[0] );
					$matches[0] = str_replace( 'decoding="async"', 'decoding="sync"', $matches[0] );
					if ( strpos( $matches[0], 'fetchpriority=' ) === false ) {
						$matches[0] = str_replace( '<img ', '<img fetchpriority="high" ', $matches[0] );
					}
				}
				return $matches[0];
			}, $html );
