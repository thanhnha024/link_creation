				...
				if ( ! empty( $command ) && ! empty( $commands[ $command ] ) && ( $base_on == 'prompt' && ! empty( $prompt ) || $base_on != 'prompt' && ! empty( $content ) ) ) {
					...
					if ( $base_on == 'prompt' ) {
						...
						$msg = strpos( $command, 'write_' ) !== false
								...
								: ( ! empty( $variations )
									...
									. $this->get_subprompt( $params )
									. ' ' . sprintf( __( 'The text to be processed is enclosed in double curly braces: %s', 'trx_addons' ), $content );
					}
					$api = OpenAi::instance();
					$response = $api->query(
						array(
					...
					if ( ! empty( $response['choices'][0]['message']['content'] ) ) {
						...
						// Get all text from the response as a single variant
							...
							$answer['data'] = array( $this->fix_headings_in_content( str_replace( array( '{{', '}}' ), '', $response['choices'][0]['message']['content'] ) ) );
						}
					} else {
