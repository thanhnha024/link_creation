...
if ( ! function_exists( 'trx_addons_sc_tgenerator_generate_text' ) ) {
	...
	function trx_addons_sc_tgenerator_generate_text() {
		...
		$system_prompt = ! empty( $settings['system_prompt'] )
							...
							: apply_filters( 'trx_addons_filter_sc_tgenerator_system_prompt', trx_addons_get_option( 'ai_helper_sc_tgenerator_system_prompt' ) );
			...
			if ( OpenAi::instance()->get_api_key() != '' && ! $demo ) {
				...
				$prompt = strpos( $command, 'write_' ) !== false
							...
							: ( ! empty( $variations )
								...
								. ' ' . sprintf( __( "The text to be processed is enclosed in double curly braces: %s", 'trx_addons' ), "{{" . preg_replace( "/(\r?\n){2,}/", '$1', $content ) . '}}' );
				...
				if ( ! empty( $response['choices'][0]['message']['content'] ) ) {
					...
					if ( ! empty( $commands[ $command ]['variations'] ) ) {
						if ( preg_match_all( '/{{(.*)}}/U', $response['choices'][0]['message']['content'], $matches ) ) {
							...
							$answer['data']['text'] = wpautop( str_replace( array( '{{', '}}' ), '', $response['choices'][0]['message']['content'] ) );
						}
