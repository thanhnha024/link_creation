				...
				?>>
								...
								if ( $settings_mode == 'full' ) {
									...
									if ( is_array( $styles_openai ) ) {
										?><div class="sc_igenerator_form_settings_field sc_igenerator_form_settings_field_style_openai<?php if ( empty( $args['model'] ) || ! Utils::is_openai_dall_e_3_model( $args['model'] ) ) echo ' trx_addons_hidden'; ?>">
											...
											<select name="sc_igenerator_form_settings_field_style_openai" id="sc_igenerator_form_settings_field_style_openai"><?php
					...
					if ( ! empty( $args['show_upload'] ) ) {
						...
						?><div class="sc_igenerator_form_field sc_igenerator_form_field_upload_image">
							<div class="sc_igenerator_form_field_inner">
								<label for="sc_igenerator_form_field_upload_image_field"><?php esc_html_e('Upload image to make variations (optional):', 'trx_addons'); ?></label><?php
								if ( $decorated ) {
									?>
								...
								?><input type="file" id="sc_igenerator_form_field_upload_image_field" class="sc_igenerator_form_field_upload_image_field" placeholder="<?php esc_attr_e( "Select an image to make variations", 'trx_addons' ); ?>"><?php
								if ( $decorated ) {
									?></div><?php
								}
							?></div>
