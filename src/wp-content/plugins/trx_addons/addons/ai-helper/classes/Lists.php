...
if ( ! class_exists( 'Lists' ) ) {
	...
	class Lists {
		 ...
		 */
		static function get_list_ai_chat_models() {
			$api_order = trx_addons_get_option( 'ai_helper_sc_chat_api_order' );
			$models = array(
			...
			return apply_filters( 'trx_addons_filter_ai_helper_list_ai_chat_models', $models );
		}
			...
			foreach( $api_order as $api => $enable ) {
				...
				if ( $api == 'stable-diffusion' && (int)$enable > 0 ) {
					...
					foreach ( $sd_upscalers as $k => $v ) {
						$upscalers[ 'stabble-diffusion/' . $k ] = $v['title'];
					}
				}
				// Stability AI
				if ( $api == 'stability-ai' && (int)$enable > 0 ) {
		...
		static function get_sd_upscalers() {
			...
			$token = trx_addons_get_option( 'ai_helper_token_stabble_diffusion' );
			if ( ! empty( $token ) ) {
				$upscalers = trx_addons_get_option( 'ai_helper_upscalers_stabble_diffusion', '' );
